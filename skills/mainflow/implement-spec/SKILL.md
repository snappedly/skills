...
| Delivery / handoff | Follow configured PR/MR creation, readiness, merge, and ticket/spec closure policy within scope. Report final SHA, required checks, axis results, finding dispositions, budget totals, PR/MR link, verified tracker states, and pending transitions with their next actor and required evidence. |
...
Create or resume the integration branch. Create or update the PR/MR at the configured event when publication is assigned to the agent; otherwise prepare the handoff for its owner. A draft needs a diff, so wait until the first integrated commit when necessary. Use `pr` to apply the configured ticket and parent-spec closing-reference rules. Include verification scope and a pointer to tdd in implementer and fix briefs for changed logic; presentation edits use visual checks. Workers may select established test boundaries; route unresolved contract decisions through the coordinator. Prose-only edits need no artificial keyword tests.

...
8. Mark the pull or merge request as ready for review once required checks pass, every applicable review axis has completed or been handled under repository policy, and every finding has the disposition required by `docs/agents/workflow.md`. Report unresolved checks or coverage gaps before proceeding. Use `/deliver` to finish provider checks, merge, production approval, deployment, and verification.

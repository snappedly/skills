...
The visual direction has to be open for this skill to have a job: a new page, a new product, a redesign, or a set of prototype variants. Where the repo already has a design language (a component library, a token file, a documented system), **that system is the brief**: take its palette, type scale, spacing, and motion as given, and spend this skill on what it leaves open, such as the shape of a hero, the hierarchy of a new screen, or the copy. Read `docs/agents/frontend.md` when the repo has one: it records which system is in play and whether new direction is wanted.
